...
6. Create `scripts/closed_lid_canary.sh` with `prepare`, `preflight`, `verify`, and `cleanup` operations. `prepare` starts a timestamped counter in an owned detached tmux session plus Amphetamine. A separate `preflight` invocation must prove that the counter survives its launching shell before any lid test. After the user closes the lid for at least 120 seconds and reopens it, `verify` proves the counter advanced by at least 90 seconds and records model, OS build, power source, and Amphetamine version in a local verification marker. Cleanup must target only the owned tmux session and must verify that normal sleep was restored before claiming success.
